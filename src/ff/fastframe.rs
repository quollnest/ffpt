...
impl FFStacks {
    ...
    pub fn encode(&mut self, qubit: usize, frame_offset: usize, pauli: PauliTuple) {
        //println!("frame: {}", frame_offset);
        self.x
            .set((qubit * self.width) + frame_offset, pauli.get_x());
        self.z
            .set((qubit * self.width) + frame_offset, pauli.get_z());
    }

    pub fn with_bitvecs(bx: BitVec, bz: BitVec, width: usize) -> Self {
        let mut ff = FFStacks {
            x: bx,
            z: bz,
            width,
    ...
    pub fn get_slice_pair(&mut self, bit1: usize, bit2: usize) -> (FFBitSlice, FFBitSlice) {
    ...
    pub fn sum_up(&self, qubit: usize, filter: &[bool]) -> (bool, bool) {
        ...
        let x_res = self.x[control_loc..control_end]
            .iter()
            .enumerate()
            .filter_map(|(i, f)| if filter[i] { Some(f) } else { None })
            .fold(false, |acc, next| acc ^ *next);

        let z_res = self.z[control_loc..control_end]
            .iter()
            .enumerate()
            .filter_map(|(i, f)| if filter[i] { Some(f) } else { None })
            .fold(false, |acc, next| acc ^ *next);
    ...
    pub fn cz(&mut self, bit_a: usize, bit_b: usize) {
        ...
        self.z[a_loc..a_end].copy_from_bitslice(self.copy_space.copy_a.as_bitslice());
        self.z[b_loc..b_end].copy_from_bitslice(self.copy_space.copy_b.as_bitslice());
    }
    ...
    pub fn cy(&mut self, control_bit: usize, target_bit: usize) {
        ...
        self.z[target_loc..target_end].copy_from_bitslice(self.copy_space.copy_b.as_bitslice());

        self.z[control_loc..control_end].copy_from_bitslice(self.copy_space.copy_c.as_bitslice());
    }
    ...
    pub fn swap(&mut self, bit_a: usize, bit_b: usize) {
        ...
        self.x[target_loc..target_end].copy_from_bitslice(self.copy_space.copy_a.as_bitslice());
        self.z[target_loc..target_end].copy_from_bitslice(self.copy_space.copy_b.as_bitslice());
        self.x[control_loc..control_end].copy_from_bitslice(self.copy_space.copy_c.as_bitslice());
        self.z[control_loc..control_end].copy_from_bitslice(self.copy_space.copy_d.as_bitslice());
    }
    ...
    pub fn zcx(&mut self, control_bit: usize, target_bit: usize) {
        ...
        self.x[target_loc..target_end].copy_from_bitslice(self.copy_space.copy_a.as_bitslice());
        self.x[control_loc..control_end].copy_from_bitslice(self.copy_space.copy_b.as_bitslice());
    }
...
impl FastFrames {
    pub fn new(num_keys: usize) -> Self {
        ...
        FastFrames {
            ffs: FFStacks::with_capacity(nkeys, nwidth),
            frames_num: 0,
            nqbits: num_keys,
        }
    }
    ...
    pub fn track_pauli_string(&mut self, string: PauliString) {
        ...
        //}
        for (i, p) in string {
            self.ffs.encode(i, self.frames_num, p);
        }
        self.frames_num += 1;
    }
    ...
    pub fn transpose(&self, _: usize) -> FastFrames {
        ...
        let mut x_vec: BitVec = BitVec::repeat(false, self.nqbits * self.frames_num);

        let mut z_vec: BitVec = BitVec::repeat(false, self.nqbits * self.frames_num);

        let mut x_copy_vec: BitVec = BitVec::repeat(false, self.nqbits);
        let mut z_copy_vec: BitVec = BitVec::repeat(false, self.nqbits);
        ...
        for fidx in 0..self.frames_num {
            ...
            for ifidx in fidx..self.frames_num {
                let xv = *self.ffs.x.get((ifidx * width) + fidx).as_deref().unwrap();

                let zv = *self.ffs.x.get((ifidx * width) + fidx).as_deref().unwrap();

                x_copy_vec.set(ifidx, xv);
                z_copy_vec.set(ifidx, zv);
            }
            x_vec[f_start..f_end].copy_from_bitslice(x_copy_vec.as_bitslice());

            z_vec[f_start..f_end].copy_from_bitslice(z_copy_vec.as_bitslice());
        }
    ...
    pub fn to_transpose_2d(&self) -> Vec<Vec<u8>> {
        ...
        for i in 0..self.frames_num {
            ...
            for j in 0..self.frames_num {
                let x = *flatten.ffs.x.get((i * width) + j).as_deref().unwrap() as u8;

                let z = *flatten.ffs.z.get((i * width) + j).as_deref().unwrap() as u8;
